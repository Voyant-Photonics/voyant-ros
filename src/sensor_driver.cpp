...
VoyantSensorDriver::VoyantSensorDriver() : Node("voyant_sensor_node")
{
    getParams();
    initialize();
...
void VoyantSensorDriver::initialize()
{
    ...
    for (int attempt = 1; attempt <= reconnect_attempts_; ++attempt)
    {
        try
        {
            RCLCPP_INFO(get_logger(), "Connecting to sensor: %s (Attempt %d/%d)", binding_address_.c_str(), attempt,
                        reconnect_attempts_);
            ...
            while (!client_->isTerminated())
            {
                const VoyantFrameWrapper *frame = client_->update();
                if (frame)
                {
...
sensor_msgs::msg::PointCloud2 VoyantSensorDriver::pointDatatoRosMsg(const VoyantFrameWrapper *frame)
{
    ...
    ros_cloud.height = 1;
    RCLCPP_INFO(get_logger(), "[+] Point cloud published");
...
void VoyantSensorDriver::publishPointCloud()
{
...

    {
        try
        {
            if (client_ && !client_->isTerminated())
            {
                const VoyantFrameWrapper *frame = client_->update();
                if (frame)
                {
            ...
            {
                RCLCPP_ERROR(get_logger(), "[-] LiDAR client invalid or terminated");
                break;
            }
        }
        catch (const std::exception &e)
